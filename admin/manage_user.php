<?php
    ...
    //   $package_sql = "SELECT * FROM packages WHERE package_type = '$category_id'";
...
?>
    <div class="container mx-auto px-4">
        ...
        <div class="main-container shadow-md rounded-lg">
            <div id="package_count" class="flex flex-wrap justify-between items-center px-4 pb-6 pt-4 bg-gray-50 border border-rose-200 font-semibold text-[1.2rem]">
                <div class="flex flex-wrap">
                    <?php 
                        $total_package = 0;
                        foreach($pack_data as $pack_datas){
                            ...
                            if($filt_date == ''){
                                $query_pack = "SELECT * FROM user WHERE package_type = '$pack' AND status = '1'";
                                $sqll = "SELECT * FROM additional_subscription WHERE package_type = '$pack' AND  date = '$formatted_date'";
                            }else{
                                $sqll = "SELECT * FROM additional_subscription WHERE package_type = '$pack' AND  date = '$formatted_date'";
                                $query_pack = "SELECT * FROM user WHERE package_type = '$pack' AND '$formatted_date' BETWEEN user.start_date AND user.end_date AND status = '1'";
                            }
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                        <tr>
                            ...
                            <th scope="col" class="py-6 px-6">
                                <span class="text-green-500">Active</span>/<span class="text-red-500">InActive</span>
                            </th>
                            <th scope="col" class="py-6 px-6">
                                Action
                            </th>
                        ...
                        <?php if ($record): ?>
                            <?php foreach ($record as $records): ?>
                                ...
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="py-4 px-6">
                                        <!-- Modal toggle -->
                                        <a href="#" type="button" id="<?php echo $records['id']; ?>" data-modal-toggle="editUserModal" class="modal-btn font-medium text-blue-600 hover:underline">Edit user</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
